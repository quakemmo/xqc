...
#define DEF_COMZONEMEGS		50 // XXX xqx changed from 24 to 50
...
void QDECL Com_Printf( const char *fmt, ... ) {
  ...
  static qboolean opening_qconsole = qfalse;
	...
	CL_ConsolePrint( msg );
// XXX xqx
	xq_write_to_logfile(msg, "console.log");
	if (strstr(msg, "[XQ] ") == msg) {
		if (strlen(msg) > 7) { // account for two color characters such as ^7 in the beginning of the string
			xq_write_to_logfile(msg+7, "xqchat.log");
		}
	}
// XXX -xqx
#endif
...
#ifndef DEDICATED
	xq_write_to_logfile(msg, "debug_console.log");
#endif
...
void Com_WriteConfigToFile( const char *filename ) {
	...
	Key_WriteBindings (f);
	MM_Key_WriteBindings (f); // XXX xqx
...
static void key_hex2bin(char *hex, char *bin) {
	for (int i = 0;  i <= 15;  i++) {
        bin[i] = (hex2dec(hex[i*2])*16) + hex2dec(hex[i*2+1]);
	}
}
static void doAES(void *data, int len, char *key, int keylen, qboolean dir) {
	char *name = "xq_decrypt";
	if (dir == qtrue) {
		name = "xq_encrypt";
	}

	if (len % 16 != 0) {
		Com_Printf("%s: data length not multiple of 16\n", name);
		return;
	}

	if (keylen != XQ_SESSION_LEN) {
		Com_Printf("%s: textual hex key length is %i - we need exactly %i\n", name, keylen, XQ_SESSION_LEN);
		return;
	}

	char binkey[XQ_SESSION_LEN/2 + 1] = {0};
	key_hex2bin(key, binkey);

	static struct AES_ctx ctx = {0};
	AES_init_ctx(&ctx, (uint8_t *)binkey); // TODO: limit to once per session
	for (int i = 0;  i < len / 16;  i++) {
		if (dir) {
			AES_ECB_encrypt(&ctx, ((uint8_t *)data) + i*16);
		} else {
			AES_ECB_decrypt(&ctx, ((uint8_t *)data) + i*16);
		}
	}
}
void xq_encrypt(void *data, int len, char *key, int keylen) {
	doAES(data, len, key, keylen, qtrue);
}
void xq_decrypt(void *data, int len, char *key, int keylen) {
	doAES(data, len, key, keylen, qfalse);
}
int xq_crypto_pad(byte *padded_data, const byte *data, int len, int pad_len) {
	Com_RandomBytes(padded_data, pad_len);

	int remainder = len % 16;
	memcpy(padded_data, data, len);
	if (!remainder) return 0;

	int add = 16 - remainder;

	//memset(padded_data + len, 0, add);
	return add;
}
// XXX -xqx
